...
## 正在进行的任务:
1.  （高阶）"page_2" 中的 "Province" 和 "City":
    1.  连接加拿大的数据
    2.  做成下拉菜单
...
2. 前端网页相关：
   ...
   3. 19/25**不用做登录**？、
   4. （这个部分**可能不需要写**，如果写的话可能也需要去其他保险公司做表单来体验一下）登录页面可以做成填写邮箱和code，下次进入时填写相同信息，进入未完成表单继续填写（？或者用户只输入邮箱，不记住code，下一次客户使用同一个邮箱，生成新的code，也能进入未完成表单。这样的难处在于生成code后是否发送到用户邮箱，如果是的话需要额外写一个邮箱发送code来实现）
